...
import static com.scaiz.utils.TestUtils.assertIllegalArgumentException;
import static com.scaiz.utils.TestUtils.assertIndexOutOfBoundsException;
import static com.scaiz.utils.TestUtils.assertNullPointerException;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertTrue;
import static org.junit.Assert.fail;

import com.scaiz.json.DecodeException;
import com.scaiz.json.JsonArray;
import com.scaiz.json.JsonObject;
import com.scaiz.utils.BufferTestUtil;
import com.scaiz.utils.RandomTestUtil;
...
public class BufferTest {
  ...
  private static final Function<byte[], Buffer> PADDED_BUFFER_FACTORY = arr -> Buffer
      .buffer(paddedByteBuf(5, arr));
  ...
  public void testAppendBuff(Function<byte[], Buffer> bufferFactory)
      throws Exception {
    ...
    assertTrue(BufferTestUtil.byteArraysEqual(bytes, b.getBytes()));
    b.appendBuffer(toAppend);
  ...
  public void testAppendBytesWithOffsetAndLen() throws Exception {
    ...
    assertTrue(BufferTestUtil.byteArraysEqual(copy, b.getBytes()));
  ...
  public void testAppendByte2() throws Exception {
    int bytesLen = 100;
    Buffer b = Buffer.buffer(BufferTestUtil.randomByteArray(bytesLen));
  ...
  public void testAppendUnsignedByte() {
    Buffer b = Buffer.buffer(BufferTestUtil.randomByteArray(100));
  ...
  public void testAppendString1() throws Exception {
    ...
    String str = BufferTestUtil.randomUnicodeString(100);
  ...
  @Test
  public void testLE() {
    checkBEAndLE(2, Buffer.buffer().appendShort(Short.MAX_VALUE),
        Buffer.buffer().appendShortLE(Short.MAX_VALUE));
    checkBEAndLE(2, Buffer.buffer().appendUnsignedShort(Short.MAX_VALUE),
        Buffer.buffer().appendUnsignedShortLE(Short.MAX_VALUE));
    checkBEAndLE(3, Buffer.buffer().appendMedium(Integer.MAX_VALUE / 2),
        Buffer.buffer().appendMediumLE(Integer.MAX_VALUE / 2));
    checkBEAndLE(4, Buffer.buffer().appendInt(Integer.MAX_VALUE),
        Buffer.buffer().appendIntLE(Integer.MAX_VALUE));
    checkBEAndLE(4, Buffer.buffer().appendUnsignedInt(Integer.MAX_VALUE),
        Buffer.buffer().appendUnsignedIntLE(Integer.MAX_VALUE));
    checkBEAndLE(8, Buffer.buffer().appendLong(Long.MAX_VALUE),
        Buffer.buffer().appendLongLE(Long.MAX_VALUE));
  }
  ...
  public void testGetOutOfBounds() throws Exception {
    ...
    assertIndexOutOfBoundsException(
        () -> b.getString(0, bytesLen + 1, "UTF-8"));
  ...
  public void testSetOutOfBounds() throws Exception {
    ...
    assertIndexOutOfBoundsException(
        () -> b.setBytes(-1, BufferTestUtil.randomByteArray(1)));
    assertIndexOutOfBoundsException(
        () -> b.setBytes(-1, BufferTestUtil.randomByteArray(1), -1, 0));
    assertIndexOutOfBoundsException(
        () -> b.setBytes(-1, BufferTestUtil.randomByteArray(1), 0, -1));
  ...
  private void testGetSetUnsignedInt(boolean isLE) throws Exception {
    ...
    for (int i = 0; i < numInts; i++) {
      if (isLE) {
        assertEquals(Integer.toUnsignedLong(Integer.MAX_VALUE + i),
            b.getUnsignedIntLE(i * 4));
      } else {
        assertEquals(Integer.toUnsignedLong(Integer.MAX_VALUE + i),
            b.getUnsignedInt(i * 4));
      }
    }
  }
  ...
  private void testGetSetMedium(boolean isLE) throws Exception {
    ...
    for (int i = 0; i < numInts; i++) {
      if (isLE) {
        assertEquals((MEDIUM_MAX_VALUE + i - MEDIUM_MAX_VALUE),
            b.getMediumLE(i * 3));
      } else {
        assertEquals((MEDIUM_MAX_VALUE + i - MEDIUM_MAX_VALUE),
            b.getMedium(i * 3));
  ...
  private void testGetSetUnsignedMedium(boolean isLE) throws Exception {
    ...
    for (int i = 0; i < numInts; i++) {
      if (isLE) {
        assertEquals(Integer.toUnsignedLong(MEDIUM_MAX_VALUE + i),
            b.getUnsignedMediumLE(i * 3));
      } else {
        assertEquals(Integer.toUnsignedLong(MEDIUM_MAX_VALUE + i),
            b.getUnsignedMedium(i * 3));
      }
    }
  }
  ...
  private void testGetSetUnsignedShort(boolean isLE) throws Exception {
    ...
    for (short i = 0; i < numShorts; i++) {
      if (isLE) {
        assertEquals(Integer.toUnsignedLong(Short.MAX_VALUE + i),
            b.getUnsignedShortLE(i * 2));
      } else {
        assertEquals(Integer.toUnsignedLong(Short.MAX_VALUE + i),
            b.getUnsignedShort(i * 2));
      }
    }
  }
  ...
  public void testGetString() throws Exception {
    String str = BufferTestUtil.randomAlphaString(100);
    Buffer b = Buffer
        .buffer(str, "UTF-8"); // encode ascii as UTF-8 so one byte per char
  ...
  public void testGetBytes() throws Exception {
    byte[] bytes = BufferTestUtil.randomByteArray(100);
    Buffer b = Buffer.buffer(bytes);
  ...
  public void testGetBytes2() throws Exception {
    ...
    assertTrue(BufferTestUtil.byteArraysEqual(sub,
        b.getBytes(bytes.length / 4, bytes.length / 4 + bytes.length / 2)));
  }
  ...
  public void testGetBytesWithByteArray() throws Exception {
    ...
    assertTrue(BufferTestUtil.byteArraysEqual(sub, result));
  }
  ...
  private void testSetBytesBuffer(Buffer buff,
      Function<byte[], Buffer> bufferFactory) throws Exception {
    Buffer b = bufferFactory.apply(BufferTestUtil.randomByteArray(100));
    buff.setBuffer(50, b);
    byte[] b2 = buff.getBytes(50, 150);
    assertEquals(b, Buffer.buffer(b2));

    byte[] b3 = BufferTestUtil.randomByteArray(100);
    buff.setBytes(50, b3);
  ...
  public void testSetBytesWithOffsetAndLen() throws Exception {
    ...
    assertTrue(BufferTestUtil.byteArraysEqual(copy, b.getBytes(1, b.length())));
  ...
  private void testSetBytesString(Buffer buff) throws Exception {
    ...
    assertNullPointerException(
        () -> Buffer.buffer(150).setString(0, null, "UTF-8"));
  ...
  public void testCopy() throws Exception {
    Buffer buff = BufferTestUtil.randomBuffer(100);
  ...
  public void testSlice1() throws Exception {
    ...
    long rand = RandomTestUtil.randomLong();
    sliced.setLong(0, rand);
    assertEquals(rand, buff.getLong(0));
    buff.appendString(BufferTestUtil.randomUnicodeString(100));
  ...
  public void testToJsonObject() throws Exception {
    ...
    buff = Buffer.buffer(BufferTestUtil.randomAlphaString(10));
    try {
      buff.toJsonObject();
      fail();
  ...
  public void testLength2() throws Exception {
    byte[] bytes = BufferTestUtil.randomByteArray(100);
    assertEquals(90,
        Buffer.buffer(Unpooled.copiedBuffer(bytes).slice(10, 90)).length());
  }
